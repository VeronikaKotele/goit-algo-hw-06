...
def flow_id_to_company_ids(flow_id: str) -> TypedDict('CompanyIDs', {'sender': str, 'receiver': str}):
    """Extracts company ID from flow ID by removing the last 4 characters"""
    company_ids = flow_id.split('-')
    if len(company_ids) < 2:
        raise ValueError(f'Invalid flow_id format: {flow_id}: should contain -')
